...
app.post('/data',(req,res)=>{
    ...
    fs.writeFile(_path+obj.name+'.txt',obj.content,(e)=>{
        ...
        res.send(`<script>alert('${obj.name}으로 저장되었습니다.');history.go(-1)</script>`)
    })
})
    ...
    fs.readdir(_path,'utf-8',(err,data)=>{
        ...
        data.forEach(v=>{
            if(v.indexOf('.')===-1){
                ...
                list += `<li class="list"><a href="/${v}">${v}</a></li>`
            }
        })
        list += '</ul>'
        list += '<button class="create_btn"><a href="http://localhost:3000/">작성</a></button>'
        list += `<style>
